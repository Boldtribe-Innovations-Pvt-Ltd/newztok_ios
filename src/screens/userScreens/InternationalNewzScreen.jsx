...
import { DISLIKE, LIKE, PRESSDISLIKE, PRESSLIKE, SAVE, SHARE, THREEDOTS, ACCOUNT, VERIFIED, RAMNABAMI } from "../../constants/imagePath";
...
console.log('International Screen - Current BASE_URL:', BASE_URL);
...
export default function InternationalNewzScreen({ navigation }) {
    ...
    const fetchInternationalNewsData = async () => {
        try {
            setLoading(true);
            console.log('Fetching international news from:', `${BASE_URL}api/news/category/international`);
            ...
            if (response.success && response.data) {
                ...
                console.log('Internationals news from API:', newsArray);
                ...
                const transformedData = newsArray.map(newsItem => {
                    ...
                    return {
                        ...
                        title: newsItem.headline || newsItem.title || "Internationals News",
                ...
                console.log('Sample of internationals news items before sorting:');
                ...
                console.log(`Total International news items with videos: ${videosCount} out of ${sortedData.length}`);

                // Log the first 3 items after sorting to verify
                console.log('internationals news order after sorting:');
                ...
                console.log('Transformed and sorted internationals News Data ready');
                setInternationalNewsData(sortedData);
            } else {
                console.error("API Error:", response.message);
                Alert.alert(
                    "Error",
                    response.message || "Unable to fetch internationals news. Please try again later.",
                    [{ text: "OK" }]
                );
                setInternationalNewsData([]);
            }
        } catch (error) {
            console.error("Error fetching internationals news:", error);
            Alert.alert(
                "Error",
                "Unable to fetch internationals news. Please check your internet connection and try again.",
                [{ text: "OK" }]
            );
            setInternationalNewsData([]);
        } finally {
    ...
    const handleLike = async (id) => {
        try {
            ...
            // Get news item
            const newsItem = internationalNewsData.find(item => item.id === id);
            if (!newsItem) {
            ...
            <View style={styles.container}>
                {loading ? (
                    <View style={styles.loadingContainer}>
                        ...
                        <Text style={{fontFamily: LORA, marginTop: HEIGHT * 0.01}}>Loading internationals news...</Text>
                    </View>
                ) : internationalNewsData.length === 0 ? (
                    <View style={styles.noNewsContainer}>
                        <Text style={styles.noNewsText}>No international News Available</Text>
                    </View>
                ) : (
                    <FlatList
                        data={prepareDataWithAds(internationalNewsData)}
