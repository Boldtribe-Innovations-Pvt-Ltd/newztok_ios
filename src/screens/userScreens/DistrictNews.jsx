...
export default DistrictNews = ({ route, navigation }) => {
    ...
    const fullImageUrl = newsData?.featured_image ? 
        ...
        (newsData?.featuredImage ?
            (newsData.featuredImage.startsWith('http') ?
                newsData.featuredImage :
                `${BASE_URL}${newsData.featuredImage.replace(/^\/+/, '')}`) :
            null);
    ...
    // Fix the title rendering by checking multiple possible property names
    const getNewsTitle = () => {
        if (newsData?.headline) return newsData.headline;
        if (newsData?.title) return newsData.title;
        if (newsData?.name) return newsData.name;
        return "News";
    };
        ...
        <>
            ...
            <MyHeader showLocationDropdown={false} onPressBack={onNavigateBack} />
                ...
                <ScrollView style={styles.container}>
                    ...
                    <Text style={styles.title}>{getNewsTitle()}</Text>
