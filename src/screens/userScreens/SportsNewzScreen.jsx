...
console.log('Sports Screen - Current BASE_URL:', BASE_URL);
...
export default function SportsNewzScreen({ navigation }) {
    ...
    const fetchSportsNewsData = async () => {
        try {
            setLoading(true);
            console.log('Fetching sports news from:', `${BASE_URL}api/news/category/sports`);
            ...
            if (response.success && response.data) {
                ...
                // No need to filter as the API already returns sports news
                console.log('Sports news from API:', newsArray);
                ...
                const transformedData = newsArray.map(newsItem => {
                    ...
                    return {
                        ...
                        title: newsItem.headline || newsItem.title || "Sports News",
                ...
                console.log('Sample of sports news items before sorting:');
                ...
                console.log(`Total sports news items with videos: ${videosCount} out of ${sortedData.length}`);

                // Log the first 3 items after sorting to verify
                console.log('Sports news order after sorting:');
                ...
                console.log('Transformed and sorted Sports News Data ready');
                setSportsNewsData(sortedData);
            } else {
                console.error("API Error:", response.message);
                Alert.alert(
                    "Error",
                    response.message || "Unable to fetch sports news. Please try again later.",
                    [{ text: "OK" }]
                );
                setSportsNewsData([]);
            }
            ...
            Alert.alert(
                "Error",
                "Unable to fetch sports news. Please check your internet connection and try again.",
                [{ text: "OK" }]
            );
            setSportsNewsData([]);
        } finally {
    ...
    const handleLike = async (id) => {
        try {
            ...
            // Get news item
            const newsItem = sportsNewsData.find(item => item.id === id);
            if (!newsItem) {
            ...
            <View style={styles.container}>
                {loading ? (
                    <View style={styles.loadingContainer}>
                        ...
                        <Text style={{fontFamily: LORA, marginTop: HEIGHT * 0.01}}>Loading sports news...</Text>
                    </View>
                ) : sportsNewsData.length === 0 ? (
                    <View style={styles.noNewsContainer}>
                        <Text style={styles.noNewsText}>No Sports News Available</Text>
                    </View>
                ) : (
                    <FlatList
                        data={prepareDataWithAds(sportsNewsData)}
