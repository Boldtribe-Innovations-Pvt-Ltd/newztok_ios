...
console.log('Entertainment Screen - Current BASE_URL:', BASE_URL);
...
export default function EntertainmentNewzScreen({ navigation }) {
    ...
    const fetchEntertainmentNewsData = async () => {
        try {
            setLoading(true);
            console.log('Fetching entertainment news from:', `${BASE_URL}api/news/category/entertainment`);
            ...
            if (response.success && response.data) {
                ...
                console.log('Entertainment news from API:', newsArray);
                ...
                const transformedData = newsArray.map(newsItem => {
                    ...
                    return {
                        ...
                        title: newsItem.headline || newsItem.title || "Entertainments News",
                ...
                console.log('Sample of entertainments news items before sorting:');
                ...
                console.log(`Total Entertainment news items with videos: ${videosCount} out of ${sortedData.length}`);

                // Log the first 3 items after sorting to verify
                console.log('entertainments news order after sorting:');
                ...
                console.log('Transformed and sorted entertainments News Data ready');
                setEntertainmentNewsData(sortedData);
            } else {
                console.error("API Error:", response.message);
                Alert.alert(
                    "Error",
                    response.message || "Unable to fetch entertainments news. Please try again later.",
                    [{ text: "OK" }]
                );
                setEntertainmentNewsData([]);
            }
        } catch (error) {
            console.error("Error fetching entertainments news:", error);
            Alert.alert(
                "Error",
                "Unable to fetch entertainments news. Please check your internet connection and try again.",
                [{ text: "OK" }]
            );
            setEntertainmentNewsData([]);
        } finally {
    ...
    const handleLike = async (id) => {
        try {
            ...
            // Get news item
            const newsItem = entertainmentNewsData.find(item => item.id === id);
            if (!newsItem) {
    ...
    const onNavigateNews = (item) => {
        ...
        navigation.navigate("Entertainments", {
            ...
            <View style={styles.container}>
                {loading ? (
                    <View style={styles.loadingContainer}>
                        ...
                        <Text style={{fontFamily: LORA, marginTop: HEIGHT * 0.01}}>Loading entertainments news...</Text>
                    </View>
                ) : entertainmentNewsData.length === 0 ? (
                    <View style={styles.noNewsContainer}>
                        <Text style={styles.noNewsText}>No entertainments News Available</Text>
                    </View>
                ) : (
                    <FlatList
                        data={prepareDataWithAds(entertainmentNewsData)}
