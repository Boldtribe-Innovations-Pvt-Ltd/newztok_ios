...
console.log('Trending Screen - Current BASE_URL:', BASE_URL);
...
export default function TrendingNewzScreen({ navigation }) {
    ...
    const fetchTrendingNewsData = async () => {
        try {
            setLoading(true);
            console.log('Fetching trending news from:', `${BASE_URL}api/news/featured`);
            ...
            if (response.success && response.data) {
                ...
                console.log('Trending news from API:', newsArray);
                ...
                const transformedData = newsArray.map(newsItem => {
                    ...
                    return {
                        ...
                        title: newsItem.headline || newsItem.title || "Trending News",
                ...
                console.log('Transformed and sorted Trending News Data ready');
                setTrendingNewsData(sortedData);
            } else {
                console.error("API Error:", response.message);
                Alert.alert(
                    "Error",
                    response.message || "Unable to fetch trending news. Please try again later.",
                    [{ text: "OK" }]
                );
                setTrendingNewsData([]);
            }
            ...
            Alert.alert(
                "Error",
                "Unable to fetch trending news. Please check your internet connection and try again.",
                [{ text: "OK" }]
            );
            setTrendingNewsData([]);
        } finally {
    ...
    const handleLike = async (id) => {
        try {
            ...
            // Get news item
            const newsItem = trendingNewsData.find(item => item.id === id);
            if (!newsItem) {
    ...
    const onNavigateNews = (item) => {
        ...
        // Navigate to the Home screen with HomeNews component
