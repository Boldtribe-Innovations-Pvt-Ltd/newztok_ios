...
import { FlatList, Image, StyleSheet, Text, TouchableOpacity, View, ScrollView, TextInput, Linking, Share, Alert } from "react-native";
import { BLACK, BLUE, GREY, RED, WHITE } from "../../constants/color";
import { MyStatusBar } from "../../components/commonComponents/MyStatusBar";
import { LIKE, SHARE, PRESSLIKE, ACCOUNT, WHATSAPP, THREEDOTS, LOGO2, VIEW } from "../../constants/imagePath";
...
export default InternationalNews = ({route, navigation }) => {
    const { newsData } = route.params;
    console.log('Received newsData in InternationalNews:', newsData);
    console.log('VideoPath in InternationalNews:', newsData?.videoPath);
    
    // Extract media content: video or image
    // Support both featured_image and featuredImage property names for compatibility
    const featuredImage = newsData?.featuredImage || newsData?.featured_image || route.params.featuredImage || '';
        ...
        if (!isLoggedIn) {
            ...
            setTimeout(() => {
                navigation.navigate("LoginSignup", { 
                    returnScreen: "InternationalNews",
                    params: { newsData }
                });
            }, 1500);
            return;
