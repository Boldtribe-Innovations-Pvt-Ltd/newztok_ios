...
export default NationalNews = ({ route, navigation }) => {
    // Add safety check for route.params
    if (!route?.params?.newsData) {
        console.error('No newsData provided to NationalNews component');
        return (
    ...
    console.log('Additional Images in NationalNews:', newsData?.additionalImage);
    ...
    const handleWhatsAppShare = async () => {
        try {
            ...
            } else if (contentType === 'youtube' && videoId) {
                console.log('🔍 Debug - Adding YouTube URL to WhatsApp share message');
                shareMessage += `🎥 Video: https://www.youtube.com/watch?v=${videoId}\n\n`;
            } else if (contentType === 'video' && newsData.videoPath) {
                const videoUrl = processUrl(newsData.videoPath);
                console.log('🔍 Debug - Adding video URL to WhatsApp share message:', videoUrl);
                shareMessage += `🎥 Video: ${videoUrl}\n\n`;
            } else {
